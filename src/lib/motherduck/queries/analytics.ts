  ...
  try {
    ...
    rating_metrics AS (
      SELECT 
        ...
        PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY label) as median_rating,
        (CAST(COUNT(CASE WHEN label >= 4 THEN 1 END) as DOUBLE) * 100.0 / NULLIF(COUNT(*), 0)) as sentiment_score,
        -- Getting recent average (last 20% of reviews)
        (SELECT AVG(CAST(label as DOUBLE))
         FROM (
           SELECT label
           FROM sample_data
