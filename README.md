...
- **Microservice for generating queries**: Currently all queries for analytics are highly coupled with code, separation of concerns and transition to microservice is required
  ...
  - [ ] Improve business insights from Groq: At present, it produces some inaccuracies due to the limitations of the open-source <code>llama</code> model, which lacks the necessary funding to enhance its capabilities.
